...
extension LineupViewModel {
    ...
    private func positionForPlayer(_ player: Player, in lineup: [FieldPosition: UUID]) -> String {
        // Search by player ID because lineup stores player IDs.
        if let position = lineup.first(where: { $0.value == player.id })?.key {

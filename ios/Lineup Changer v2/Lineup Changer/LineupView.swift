...
struct LineupOrderView: View {
    ...
    // Builds the player label shown throughout the lineup screen.
    private func lineupDisplayLabel(for player: Player) -> String {
        ...
        // Base label changes depending on whether full names/numbers are enabled.
        ...
        // Guest status is included in the returned label when applicable.
        return player.status == .guest ? "\(baseLabel) (Guest)" : baseLabel
    }

    // Shared styling helper for section headers used in the lineup screen.
